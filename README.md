...
않습니다. 이미 사용한 횟수·일수는 **입력하기 전에는 계산하지 않습니다** — 모르는 값을 0으로
추정하면 한도가 통째로 사라져 보험금이 실제보다 많게 표시됩니다. 사용한 통원이 없으면 `0`을
직접 입력하시면 되고, 음수·소수·문자 같은 값은 고쳐서 계산하지 않고 그대로 막습니다.
100을 넘는 값은 유효한 과거 상태로 그대로 받습니다. 비중증은 보상 단위가 '통원 1일당(외래 및 처방·조제비 합산)'이므로 **같은 날 청구는 한
행으로 합쳐** 입력해야 일수가 실제보다 빨리 소진되지 않습니다.

진료비는 있으나 **지급 보험금이 0원인 통원**이 그 한도를 소진하는지는 약관에 판단 문언이
없습니다(표는 '통원 100회'·'통원 100일'이라 통원 자체를, 제5조④는 '보상한 횟수'·'보상한 일수'라
보상된 건을 가리키는 것으로 읽힙니다). 계산기는 한쪽으로 단정하지 않고 **두 해석을 모두 계산해
결과가 갈릴 때만** 묶음 전체를 미계산으로 막습니다. 결과가 같으면 정상 계산합니다. 중증 100회와
비중증 100일에 같은 방식을 쓰고, 같은 성격의 문제가 있는 3대비급여 횟수도 같은 방식으로 막습니다.
**세 규칙을 하나로 합치지는 않았습니다** — 원문 표현이 각각 '통원 100회'·'통원 100일'·'보상한
횟수'로 다르고 카운터의 단위도 회와 일로 갈리므로, 근거·상수·카운터·안내 문구를 모두 분리해
둡니다. 해제 조건은 `docs/insurance/audit-status.md`에 있습니다.
...
상한을 반영합니다. **외래 횟수와 처방전 건수는 별개 한도**라 입력도 따로 받으며, 해당하는
통원 행이 있으면 **입력하기 전에는 계산하지 않습니다** — 모르는 값을 0으로 추정하면 한도가
통째로 사라져 보험금이 실제보다 많게 표시됩니다. 이전 이용이 없으면 `0`을 직접 입력하시면
되고, 음수·소수·문자 같은 값은 고쳐서 계산하지 않고 그대로 막습니다. 180을 넘는 값은 유효한
과거 상태로 그대로 받습니다. 입원만 있는 청구에는 두 입력을 묻지 않습니다. 4세대는 동일한 보장축의 청구를 묶어 회당 20만 원, 비급여 통원 100회,
증권상 연간 가입금액, 3대비급여의 항목별 금액·횟수 한도를 반영합니다. 계약마다 다른 가입금액은
증권에서 확인한 값을 입력한 경우에만 적용합니다. 4세대의 **이미 사용한 횟수**는 세 축이 따로
있습니다 — 비급여 통원 100회, 도수치료·체외충격파·증식치료 50회, 비급여 주사료 50회입니다.
비급여 MRI·MRA는 금액 한도만 있고 횟수 한도가 없어 묻지 않습니다. 세 축 모두 **입력하기
전에는 계산하지 않습니다** — 모르는 값을 0으로 추정하면 한도가 통째로 사라져 보험금이 실제보다
많게 표시됩니다. 받은 치료가 없으면 `0`을 직접 입력하시면 되고, 음수·소수·문자 같은 값은
고쳐서 계산하지 않고 그대로 막습니다. 한도를 넘는 값은 유효한 과거 상태로 그대로 받습니다. 5세대는 회당·일당 한도와 비급여 연간 보험금
