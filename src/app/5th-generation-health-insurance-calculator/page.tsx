    ...
    <article className="container-base py-10">
      <header className="mb-8">
        ...
        <p className="mt-3 text-slate-600 leading-relaxed">
          ...
          비급여에서 제외되고, <b>상급병실료 차액</b>도 입원 의료비와 산식이 다릅니다.
          별도 보장종목은 연간 금액·횟수 한도가 함께 걸려 있어 1건만으로는 계산할 수 없으므로,
          <b>단건 계산은 차단</b>하고 아래 <b>여러 건 합산 계산</b>에서 계산합니다.
          <b>상급병실료 차액</b>은 입원일수 축이 필요해 두 계산기 모두 차단합니다.
        </p>
      </header>
      ...
      <div className="mt-6">
        <NoticeBox variant="info">
          ...
          <br />
          단건 계산의 범위는 특별약관1·2의 <b>(1)상해비급여·(2)질병비급여</b>입니다. 중증의 3대비급여
          (근골격계 이학요법·체외충격파, 주사료, MRI)와 비중증의 비급여 자기공명영상진단은 약관이
          위 보장종목에서 명시적으로 제외하며 공제금액·금액한도·횟수한도가 다릅니다. 이 항목들은
          단건 계산에서 <b>현재 계산하지 않고</b>, 여러 건 합산 계산에서 별도 산식으로 계산합니다.
          <br />
          비중증의 근골격계 이학요법·체외충격파와 주사료, 그리고 항암제·항생제(항진균제 포함)·
          희귀의약품을 위해 사용된 중증 비급여 주사료는 약관이 반대로 <b>(1)상해비급여·(2)질병비급여
          에서 보상</b>하므로, 여러 건 합산 계산이 일반 비급여 산식으로 계산합니다.
          <br />
          상급병실료 차액은 입원 보상 대상인 &lsquo;비급여 의료비&rsquo;에서 제외되고 별도
          산식(비급여 병실료의 50%, 1일 평균 10만 원 한도)이 적용되어 <b>두 계산기 모두 계산 대상이
          아닙니다</b>. 해당 청구는 보험사 안내를 확인해 주세요.
        </NoticeBox>
      </div>
