    ...
    : coverage === "benefit"
      ...
      ? nhisRateNum === null
        ...
        : calc2026({
            ...
            coverage: "benefit",
            visit,
            // ⚠ 종별은 급여 **통원**의 최소공제만 가른다(G-34C). 급여 입원의 자기부담률은
            //   약관이 20%로 정하고 있어 엔진이 종별을 읽지 않는다. 종전에는 두 경로 모두에
            //   실어 보냈고 입원에서는 **읽고 무시**됐다.
            //   ⚠ 상태(`benefitTier`)와 선택 UI는 그대로 둔다 — 통원↔입원을 오가도 선택이
            //     보존되고, 통원으로 돌아오면 같은 값이 다시 전달된다.
        ...
        {coverage === "non_benefit" && nonBenefitItem === "general" && visit === "outpatient" && (
          <div className="sm:col-span-2 max-w-md">
            ...
            <RawAmountInput
              ...
              ariaLabel="통원 가입금액"
            />
            {/* ⚠ 종전 문구는 "0원을 입력해도 미입력으로 처리합니다"였다. G-24가 명시적 `0`에
                   **미입력과 분리된 결과 안내**를 붙이면서 이 설명이 결과 화면과 어긋났다
                   (계산은 두 경우 모두 한도 미적용으로 같다 — 달라진 것은 안내다).
                   ⚠ 0원 가입이 약관상 유효한지, 실제 한도가 0원인지는 단정하지 않는다.
                   여기서 말하는 것은 **계산기가 무엇을 하는지**뿐이다. */}
            <p className="mt-2 text-xs text-slate-500">
              약관은 통원 가입금액을 <b>20만 원 이내에서 계약 시 정한 금액</b>으로 규정합니다
              (중증은 1회당, 비중증은 1일당). 계약마다 다른 값이라
              완전히 비우거나 0원을 입력하면 계산기에서는 이 한도를 적용하지 않습니다.
              0원을 입력한 경우 그 사실을 결과 안내에 따로 표시합니다.
            </p>
          </div>
        )}
        ...
        <div className="mt-8 space-y-4">
          ...
          {!amountInvalid && outpatientLimitInvalid && (
            <NoticeBox variant="warning">
              ...
              <b> 200000</b> 또는 <b>200,000</b> 형식입니다.
              <b> 완전히 비우거나 0원을 입력하면</b> 계산기에서는 이 한도를 적용하지 않습니다.
              0원을 입력한 경우 그 사실을 결과 안내에 따로 표시합니다.
              공백만 입력한 값은 빈 값으로 보지 않습니다.
