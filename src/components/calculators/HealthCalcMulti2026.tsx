...
export default function HealthCalcMulti2026() {
  ...
  const result = coverage === "benefit"
    ...
    : needsItem
      ? null
      : calculateMany2026({
          cause, coverage: "non_benefit", visit, severity, tier,
  ...
  return <div className="card mt-8">
    ...
    <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4">
      ...
      {coverage === "non_benefit" && nonBenefitItem === "general" && <label className="text-sm font-semibold">질환 구분<select className="input-base mt-1" value={severity} onChange={(e) => setSeverity(e.target.value as Severity)}><option value="critical">중증</option><option value="non_critical">비중증</option></select></label>}
