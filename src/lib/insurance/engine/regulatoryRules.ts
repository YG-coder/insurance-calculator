...
export const REGULATORY_RULES = {
  ...
  // ⚠ 지급 보험금이 0원인 통원이 이 100**회**를 소진하는지는 원문에 판단 문언이 없다.
  //   표는 '통원 100회'(제3조 (1)①·(2)① 인쇄 p.258·261)라 통원 자체를, 제5조④(p.280)는
  //   '보상한 횟수'라 보상된 건을 가리키는 것으로 읽혀 갈린다.
  ...
  GEN2026_CRITICAL_OUTPATIENT_VISITS_ON_ZERO_PAY: held(
    ...
    "확인된 것: 중증 일반 비급여 통원의 연간 한도는 통원 100회이고 단위는 '회'다(GEN2026-CRITICAL-OUTPATIENT-ANNUAL-VISITS = CONFIRMED). 같은 날 외래+처방과 같은 치료 목적의 하루 2회 이상 통원은 '1회의 통원'으로 본다(제3조 ⑥⑦ p.259 / ⑤⑦ p.262). 확인되지 않은 것: 공제금액이 의료비 이상이거나 연간 보험가입금액이 소진돼 지급 보험금이 0원인 통원이 그 100회를 소진하는지. 표의 '통원 100회'는 통원 자체를, 제5조④의 '보상한 횟수'는 보상된 건을 가리키는 것으로 읽혀 한쪽으로 단정할 수 없다. 값을 만들지 않고, 두 해석을 독립 실행해 결과가 갈릴 때만 묶음 전체를 차단한다",
    "2026-09-03",
  ),
  ...
  //   (인쇄 p.264~265)와 이월·한도 조문(제3조(3)⑦ p.267·제5조④ p.280)이 모두 '보상한 횟수'만
  ...
  GEN2026_SPECIAL_ITEM_COUNT_ON_ZERO_PAY: held(
    ...
    "확인된 것: 금액 축의 누적 대상은 '지급한 금액'이고 1회의 단위는 치료·진단행위(주사료는 1회 통원·입원)다. 승인 구간의 카운터 단위는 '치료횟수'로 확정됐다(GEN2026-MSK-APPROVAL-COUNT-BASIS). 확인되지 않은 것: 공제금액이 의료비 이상이어서 지급 보험금이 0원인 치료행위가 <표1>의 '50회까지 보상'과 제3조(3)⑦·제5조④의 '보상한 횟수'를 소진하는지. 원문·예시·정의 어디에도 판단 근거가 없어 값을 만들지 않는다. ⚠ 이 HOLD는 3대비급여 횟수 축에만 해당한다 — 비중증 통원의 '통원 100일'은 별개 규칙(GEN2026-NONCRITICAL-OUTPATIENT-DAYS-ZEROPAY)이고, 중증 통원의 '통원 100회'는 아직 이 계열의 규칙으로 등록돼 있지 않다",
    "2026-09-03",
  ),
