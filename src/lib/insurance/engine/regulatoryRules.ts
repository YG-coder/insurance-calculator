...
/**
 * 특약1 제5조⑤ — 500만원 **공제금액** 상한 조항 하나만 가리키는 전용 출처.
 *   ⚠ `GEN5_CRITICAL_LIMIT_TERMS`(제5조②③⑤ 묶음)와 따로 둔다. 그쪽은 통원 가입금액
 *     상한선 등 다른 규칙도 함께 쓰고 검색 페이지 주소를 갖고 있어, 이 조항의 적용 범위를
 *     특정하는 근거로 쓰기에는 위치가 넓다.
 */
const GEN5_CRITICAL_DEDUCTIBLE_CAP_TERMS: RegulatorySource = {
  ...GEN5_DIRECT_BASE,
  locator: "별표/서식 → [별표 15] 표준약관(별표 식별번호 3216359) → 실손의료보험 특별약관1(중증 비급여 실손의료비) 제5조(보험가입금액 한도 등) 제5항, 2026. 5. 6. 공포·시행본 인쇄 p.280",
};

/**
 * 특약1 제3조(3)⑦·제5조④ — **계속 중인 치료의 이월 한도** 조항.
 *   금액은 "지급한 금액", 횟수는 "보상한 횟수"로 차감한다.
 *   ⚠ 연간 한도 일반의 소진 기준이 아니다. 제5조④는 "…제7항에 따른 계속 중인 입원 또는
 *     통원의 보장한도는…"으로 참조 조항을 명시한다(2026-09-04 전문 재직독).
 */
...
/**
 * 상급병실료 차액 — 특약1·2의 (1)상해비급여·(2)질병비급여 <구분·보상금액> 안의 **한 행**이다.
 * 3대비급여처럼 독립된 (3) 보장종목이 아니다. 네 표의 문언이 동일하다.
 */
const GEN5_ROOM_CHARGE_TABLES: RegulatorySource = {
  ...GEN5_DIRECT_BASE,
...
/**
 * 특약2 제5조④ — **계속 중인 통원의 이월** 단위가 '일수'임을 재확인한다.
 *   ⚠ 이 조항만으로는 연간 100일 한도의 소진 기준이 정해지지 않는다(2026-09-04 전문 재직독).
 */
...
/**
 * 중증 통원 연 100회 관련 문언이 실제로 있는 자리.
 *   ①표: "매년 계약해당일부터 1년간 통원 100회를 한도로 합니다"(인쇄 p.258·p.261)
 *        → 연간 한도를 정하는 조항이며 세는 대상은 '통원' 자체다.
 *   ②제5조④: "연간 보장한도(횟수)에서 … 보상한 횟수를 차감한 잔여 횟수"(인쇄 p.280)
 *        → ⚠ **계속 중인 입원·통원의 이월 한도** 전용이다. 연간 한도 일반의 소진 기준으로
 *          확대해 인용하지 않는다(2026-09-04 전문 재직독).
 *   ③같은 날 합산: "1회의 통원으로 보아"(인쇄 p.259·p.262)
 */
const GEN5_CRITICAL_OUTPATIENT_VISIT_TERMS: RegulatorySource = {
  ...GEN5_DIRECT_BASE,
  locator: "별표/서식 → [별표 15] 표준약관(별표 식별번호 3216359) → 실손의료보험 특별약관1 제3조 (1)상해비급여 제1항(인쇄 p.258)·(2)질병비급여 제1항(인쇄 p.261) <구분·보상금액> 통원 행 '통원 100회', 같은 조 (1)제6항·제7항(p.259)·(2)제5항·제7항(p.262) '1회의 통원으로 보아', 제5조(보험가입금액 한도 등) 제4항(p.280) '연간 보장한도(횟수)…보상한 횟수', 2026. 5. 6. 공포·시행본",
};

/** 상급병실료 차액의 정의 — 특약1 제2조(용어의 정의). */
const GEN5_ROOM_CHARGE_DEFINITION: RegulatorySource = {
  ...GEN5_DIRECT_BASE,
  locator: "별표/서식 → [별표 15] 표준약관(별표 식별번호 3216359) → 실손의료보험 특별약관1 제2조(용어의 정의) '상급병실료 차액'·'보장대상의료비', 2026. 5. 6. 공포·시행본 인쇄 p.257",
};

/** 상급병실료 차액이 연간 보험가입금액을 일반 (1)(2)와 공유한다는 근거. */
const GEN5_ROOM_CHARGE_ANNUAL_BASIS: RegulatorySource = {
  ...GEN5_DIRECT_BASE,
  locator: "별표/서식 → [별표 15] 표준약관(별표 식별번호 3216359) → 실손의료보험 특별약관1 제5조 제1항(인쇄 p.279)·특별약관2 제5조 제1항(인쇄 p.308) — (1)상해비급여·(2)질병비급여 각각에 대하여 입원과 통원의 보상금액을 합산한 연간 보험가입금액, 2026. 5. 6. 공포·시행본",
};

/**
 * ⚠ **보조 근거**다. 5세대 표준약관 원문이 아니라 동일 문언의 **적용 순서 확인용**으로만 쓴다.
 *   원문은 "비급여 병실료의 50%. 다만, 1일 평균금액 10만원을 한도로 하며, 1일 평균금액은
 *   입원기간 동안 비급여 병실료 전체를 총 입원일수로 나누어 산출합니다."인데,
 *   앞 문장의 주어(보상금액)에 한도가 걸리는지 뒤 정의문(병실료)에 걸리는지가 문언만으로
 *   한 가지로 읽히지 않는다. 보험사 공식 안내의 계산 예시가 **50%를 먼저 적용한 뒤**
 *   1일 10만원 보험금 한도를 거는 순서를 보여준다.
 *     예1) 10일·1일 차액 18만원 → (20만−2만)×50%×10일 = 90만원 (1일 평균 9만원)
 *     예2) 5일·1일 차액 30만원 → 1일 평균 15만원이라 1일 10만원 한도로 50만원
 */
const SAMSUNG_ROOM_CHARGE_EXAMPLE: RegulatorySource = {
  document: "삼성화재 라이프케어 — 상급병실료 차액 보상 안내 및 계산 예시 (보조 근거: 적용 순서 확인용, 5세대 표준약관 원문 아님)",
  issuer: "삼성화재해상보험",
  publishedOrEffective: "2026-09-03",
  url: "https://m.samsungfire.com/m2/lifecare/contents/life01.html",
  locator: "'의료실비보험 기준 및 상급 병실 보상 내용' — 산식과 보상사례 2건(1일 평균금액 10만원 이하·초과)",
};
...
/**
 * 2026-09-05 별표15 2026.5.6 공포·시행본 특별약관1 제5조 제5항(인쇄 p.280)을 재직독한 규칙.
 *   같은 조항을 2026.7.15 시행본(별표 식별번호 3265643)·2026.9.10 시행본에서도 열어
 *   문언이 동일함을 대조했다. **기준 판본은 2026.5.6 그대로 둔다.**
 */
const confirmed0905 = <T>(
  ruleId: string,
  value: T,
  sources: readonly RegulatorySource[],
  note?: string,
) => confirmed(ruleId, "2026", value, sources, note, "2026-09-05");
...
export const REGULATORY_RULES = {
  ...
  // ⚠ 지급 보험금이 0원인 통원이 이 100**회**를 소진하는지는 직접 읽은 범위에서 판단 문언을
  //   찾지 못했다. 표는 '통원 100회'(제3조 (1)①·(2)① 인쇄 p.258·261)라 **통원 자체**를 센다.
  //   ⚠ 제5조④(p.280)의 '보상한 횟수'를 반대 해석의 근거로 들지 않는다 — 그 조항은
  //     계속 중인 입원·통원의 **이월 한도** 전용이다(아래 note 참조).
  //   ⚠ 비중증의 '통원 100일'(GEN2026-NONCRITICAL-OUTPATIENT-DAYS-ZEROPAY)과 3대비급여의
  //     '보상한 횟수'(GEN2026-SPECIAL-ITEM-COUNT-ZEROPAY)는 조문도 단위도 달라 **별개 규칙**이다.
  //     하나로 합치지 않는다. 한도값 100과 단위 '회'는 CONFIRMED로 이미 등록돼 있다.
  GEN2026_CRITICAL_OUTPATIENT_VISITS_ON_ZERO_PAY: held(
    "GEN2026-CRITICAL-OUTPATIENT-VISITS-ZEROPAY", "2026",
    [GEN5_CRITICAL_OUTPATIENT_VISIT_TERMS],
    "원문 사실: 중증 일반 비급여 통원의 연간 한도는 통원 100회이고 단위는 '회'다(GEN2026-CRITICAL-OUTPATIENT-ANNUAL-VISITS = CONFIRMED). 표(제3조 (1)①·(2)① p.258·261)가 세는 대상은 '통원' 자체다. 같은 날 외래+처방과 같은 치료 목적의 하루 2회 이상 통원은 '1회의 통원'으로 본다(제3조 ⑥⑦ p.259 / ⑤⑦ p.262). ⚠ 제5조④는 **계속 중인 입원·통원의 이월 한도** 조항이다 — 2026-09-04 전문 재직독 결과 '제3조 (1)상해비급여 제3항 또는 제4항, (2)질병비급여 제2항 또는 제3항 및 (3)3대비급여 제7항에 따른 계속 중인 입원 또는 통원의 보장한도는…'으로 참조 조항이 명시돼 있다. 연간 한도 일반의 소진 기준을 정하는 조항이 아니므로 그렇게 확대해 인용하지 않는다. 확인하지 못한 것: 공제금액이 의료비 이상이거나 연간 보험가입금액이 소진돼 지급 보험금이 0원인 통원이 그 100회를 소진하는지. **직접 읽은 범위**(위 출처의 표·조문)에서 판단 문언을 찾지 못했다 — 감독당국 유권해석·금융분쟁조정례·보험사 지급심사 기준은 조회하지 않았으므로 근거가 존재하지 않는다고 단정하지 않는다. 계산기의 현재 정책(보수적 차단): 값을 만들지 않고, 두 해석을 독립 실행해 결과가 갈릴 때만 묶음 전체를 차단한다. 근거 서술을 정정했다고 해서 이 차단을 완화하지 않는다",
    "2026-09-03",
  ),
  ...
  // ⚠ 이 규칙이 확정하는 것은 **값(500만원)과 적용 조건(입원·상급종합/종합·공제금액·계약해당일
  //    기산)**뿐이다. 조문이 열거한 "상해·질병 및 3대비급여 의료비"를 **하나로 합산해** 세는지,
  //    보장종목·원인별로 각각 세는지는 이 규칙이 정하지 않는다 —
  //    GEN2026-CRITICAL-DEDUCTIBLE-POOL-SCOPE(HOLD)에서 따로 추적한다.
  GEN2026_CRITICAL_ANNUAL_DEDUCTIBLE_CAP: confirmed0905(
    "GEN2026-CRITICAL-ANNUAL-DEDUCTIBLE-CAP", 5_000_000, [GEN5_CRITICAL_DEDUCTIBLE_CAP_TERMS],
    "제5조⑤ 문언(2026-09-05 직독, 인쇄 p.280): “제3조(보장종목별 보상내용)에서 정한 입원의 경우 상급종합병원·종합병원의 상해·질병 및 3대비급여 의료비(3대비급여 중 근골격계 이학요법치료·체외충격파치료 및 주사료 관련 비급여의료비는 제외) 중 공제금액이 계약일 또는 매년 계약해당일부터 기산하여 연간 500만원을 초과하는 때에는 500만원까지 공제합니다.” 확정한 것: ①값 500만원 ②입원 한정 ③상급종합·종합병원 한정 ④누적 대상은 자기부담금이 아니라 공제금액 ⑤기산점은 계약일 또는 매년 계약해당일 ⑥3대비급여 중 근골격계 이학요법·체외충격파·주사료는 괄호로 제외 ⑦특별약관2(비중증) 제5조에는 같은 항이 없다(인쇄 p.308~310 전수 확인). 확정하지 않은 것: 열거된 의료비의 합산 범위(GEN2026-CRITICAL-DEDUCTIBLE-POOL-SCOPE 참조). 같은 문언을 2026.7.15 시행본(별표 식별번호 3265643)·2026.9.10 시행본에서도 대조해 동일함을 확인했고, 기준 판본은 2026.5.6 시행본을 유지한다",
  ),
  ...
  // ⚠ 승인 구간의 카운터 단위만 확정한다. <표1> 주)는 "각 **치료횟수**를 합산하여 최초 10회
  //   보장"이라고 쓰므로, 최초 10회와 이후 10회 단위 구간은 **치료행위 수**로 센다.
  //   지급 보험금이 0원이어도 치료를 받았으면 이 구간을 소진한다.
  //   ⚠ 연간 50회 한도의 "보상한 횟수"는 이 규칙으로 확정되지 않는다 —
  //     그쪽은 GEN2026-SPECIAL-ITEM-COUNT-ZEROPAY로 HOLD다.
  GEN2026_MSK_APPROVAL_COUNT_BASIS: confirmed5th(
    "GEN2026-MSK-APPROVAL-COUNT-BASIS", "treatment_acts", [GEN5_MSK_APPROVAL_NOTE],
    "<표1> 주)가 '각 치료횟수를 합산하여 최초 10회 보장'이라고 규정한다. 승인 구간(최초 10회·이후 10회 단위)의 카운터 **단위**는 지급 보험금이 아니라 치료행위다. ⚠ 확정한 것은 단위뿐이다 — ①연간 50회 한도의 '보상한 횟수'에 지급 0원 행위가 포함되는지는 확정하지 않는다(GEN2026-SPECIAL-ITEM-COUNT-ZEROPAY = HOLD) ②계약해당일 이후 이미 받은 **과거** 치료행위 수를 피보험자가 어떻게 확인하는지도 약관에 없다(GEN2026-MSK-APPROVAL-PRIOR-ACT-COUNT = HOLD)",
  ),
  // ⚠ 승인 구간은 '치료횟수'로 세는데, 계약해당일 이후 **이미 받은 치료행위 수**를 피보험자가
  //   확인하는 공식 기준이 약관에 없다. 직접 읽은 범위에서는 <표1> 주)·<보상기간 예시>
  //   (인쇄 p.264~265)와 **이월 조문**(제3조(3)⑦ p.267·제5조④ p.280)이 모두 '보상한 횟수'만
  //   제시한다. 두 수는 지급 0원 치료행위가 있으면 갈라지므로 서로 대신 쓸 수 없다.
  //   → 값을 만들지 않고, 사용자가 보험사에서 확인한 값을 별도 입력축으로 받는다.
  GEN2026_MSK_APPROVAL_PRIOR_ACT_COUNT: held(
    "GEN2026-MSK-APPROVAL-PRIOR-ACT-COUNT", "2026",
    [GEN5_MSK_APPROVAL_NOTE, GEN5_THIRD_CARRYOVER],
    "확정된 것(CONFIRMED): 승인 구간의 카운터 단위는 '치료횟수'다(GEN2026-MSK-APPROVAL-COUNT-BASIS). / 보류 중인 것(HOLD): 피보험자가 계약해당일 이후 이미 받은 치료행위 수를 확인하는 **공식 서류·기준**. 직접 읽은 범위에서 약관은 <표1> 주)·<보상기간 예시>(p.264~265)와 **계속 중인 치료의 이월 조문**(제3조(3)⑦ p.267·제5조④ p.280) 모두 '보상한 횟수'만 제시하고, '치료횟수'를 확인하는 방법은 정하지 않는다. 지급 0원 치료행위가 있으면 두 수가 갈라지므로 '보상한 횟수'를 치료행위 수로 대신 쓸 수 없다. / 계산기의 현재 정책: 두 축을 섞지 않기 위해 사용자가 보험사에서 별도로 확인한 값을 독립 입력축 priorAnnualTreatmentActCount로 받아 승인 구간에만 쓰고, 미입력이면 0으로 추정하지 않고 묶음 전체를 차단한다(확인된 0회는 유효값). 이 축은 근골격계에만 열고 다른 항목에 실리면 값이 0이어도 차단한다. 공식 확인 기준이 정해지기 전까지 계산기가 과거분을 스스로 알아낸다고 주장하지 않는다",
    "2026-09-03",
  ),
  ...
  GEN2026_SPECIAL_ITEM_CARRYOVER_BASIS: confirmed5th(
    ...
    "제3조(3)⑦·제5조④ — **계속 중인 치료의 이월** 계산에서 연간 보장한도(금액)에서 '지급한 금액'을, 연간 보장한도(횟수)에서 '보상한 횟수'를 차감한다. <보상기간 예시>도 '지급된 보험금'과 '보상한 횟수'를 나눠 적는다. ⚠ 규칙 이름대로 **이월 기준**이며, 보험기간 중 연간 한도의 소진 기준을 정하는 것이 아니다",
  ),

  // ── 상급병실료 차액 ─────────────────────────────────────────────
  //   ⚠ 3대비급여처럼 독립된 (3) 보장종목이 아니라 (1)(2) 표 안의 한 행이다.
  //     따라서 상해·질병이 나뉘고, 연간 보험가입금액을 일반 (1)(2)와 공유한다.
  GEN2026_ROOM_CHARGE_PAY_RATE: confirmed5th(
    "GEN2026-ROOM-CHARGE-PAY-RATE", 0.5, [GEN5_ROOM_CHARGE_TABLES],
    "'상급병실료 차액 — 비급여 병실료의 50%'. 자기부담은 나머지 50%이며 별도 정액 최소공제가 없다",
  ),
  GEN2026_ROOM_CHARGE_DAILY_PAY_CAP: confirmed5th(
    "GEN2026-ROOM-CHARGE-DAILY-PAY-CAP", 100_000, [GEN5_ROOM_CHARGE_TABLES],
    "'다만, 1일 평균금액 10만원을 한도로 하며, 1일 평균금액은 입원기간 동안 비급여 병실료 전체를 총 입원일수로 나누어 산출합니다'",
  ),
  // 값이 아니라 **적용 순서**를 고정한다. 순서를 뒤집으면 지급액이 최대 2배 달라진다.
  GEN2026_ROOM_CHARGE_DAILY_CAP_BASIS: confirmed5th(
    "GEN2026-ROOM-CHARGE-DAILY-CAP-BASIS", "insurance_pay",
    [GEN5_ROOM_CHARGE_TABLES, SAMSUNG_ROOM_CHARGE_EXAMPLE],
    "1일 10만원 한도는 **보상금액**에 건다 — 차액의 50%를 먼저 계산한 뒤 1일 평균 보험금이 10만원을 넘지 않도록 자른다. 원문 문언은 앞 문장의 주어(보상금액)에 걸리는 것으로 읽히지만 뒤 정의문이 병실료 기준이라 한 가지로 단정되지 않아, 동일 문언을 해석한 보험사 공식 안내의 계산 예시로 순서를 확인했다. ⚠ 보조 근거는 5세대 원문이 아니며 적용 순서 확인 외의 값 근거로 쓰지 않는다",
  ),
  GEN2026_ROOM_CHARGE_SHARES_ANNUAL_LIMIT: confirmed5th(
    "GEN2026-ROOM-CHARGE-SHARES-ANNUAL-LIMIT", true,
    [GEN5_ROOM_CHARGE_ANNUAL_BASIS, GEN5_ROOM_CHARGE_TABLES],
    "상급병실료 차액은 (1)(2) 표 안의 행이므로 제5조① 연간 보험가입금액(중증 5천만원 이내·비중증 1천만원 이내)을 일반 입원·통원 보상금액과 **공유**한다. 3대비급여처럼 별도 한도를 갖지 않는다",
  ),
  GEN2026_ROOM_CHARGE_CAUSE_SEPARATED: confirmed5th(
    "GEN2026-ROOM-CHARGE-CAUSE-SEPARATED", true,
    [GEN5_ROOM_CHARGE_ANNUAL_BASIS, GEN5_ROOM_CHARGE_TABLES],
    "제5조①이 (1)상해비급여·(2)질병비급여 각각에 대해 가입금액을 정하므로 상해·질병 축이 **분리**된다. 3대비급여의 '각 상해·질병 치료행위를 합산하여'와 반대다",
  ),
  GEN2026_ROOM_CHARGE_EXCLUDED_FROM_INPATIENT_MEDICAL: confirmed5th(
    "GEN2026-ROOM-CHARGE-EXCLUDED-FROM-INPATIENT-MEDICAL", true,
    [GEN5_ROOM_CHARGE_TABLES, GEN5_ROOM_CHARGE_DEFINITION],
    "같은 표의 입원 행이 '비급여 의료비(비급여 병실료는 제외합니다)'로 규정하고, 제2조 정의도 보장대상의료비에서 '비급여 병실료 중 회사가 보장하지 않는 금액'을 뺀다. 일반 입원 의료비와 병실료 차액은 입력을 합치면 안 된다",
  ),
  // ⚠ 제5조⑤(500만원 공제금액 상한)가 상급병실료 차액에도 적용되는지는 근거를 찾지 못했다.
  //   상급병실료 행에는 약관상 '공제금액'이 규정돼 있지 않고(통원 <표1>에만 쓰인다),
  //   제2조 정의는 병실료를 보장대상의료비에서 분리한다. 값을 만들지 않는다.
  GEN2026_ROOM_CHARGE_DEDUCTIBLE_POOL: held(
    "GEN2026-ROOM-CHARGE-DEDUCTIBLE-POOL", "2026",
    [GEN5_ROOM_CHARGE_TABLES, GEN5_ROOM_CHARGE_DEFINITION, GEN5_CRITICAL_LIMIT_TERMS],
    "확인된 것(2026-09-03 제5조⑤ 직독, 인쇄 p.280): ①500만원 상한은 특별약관1(중증) 전용이고 특별약관2 제5조에는 같은 조항이 없다(p.309). ②대상은 상급종합병원·종합병원 입원뿐이다. ③3대비급여 중 근골격계 이학요법·체외충격파·주사료는 괄호로 제외되고 MRI는 포함된다. ④상급병실료 차액 행에는 '공제금액'이라는 용어가 없고 보상금액(50%)만 규정된다. 확인되지 않은 것: ①상급병실료 차액의 미지급 50%가 제5조⑤의 '공제금액'에 포함되는지 ②1일 평균 보험금 한도 초과분이 포함되는지 ③기본병실료 관련 제외액이 포함되는지 ④포함된다면 일반 입원 공제금액과의 누적 순서. 네 가지 중 어느 것도 원문·유권해석·판매약관에서 확인하지 못해 pool을 적용하지 않는다",
    "2026-09-03",
  ),
  // ⚠ 제5조⑤가 열거한 "상해·질병 및 3대비급여 의료비"의 **합산 범위**는 별도 규칙이다.
  //    위 상급병실료 HOLD와 합치지 않는다 — 그쪽은 "상급병실료 차액의 미지급분이 공제금액에
  //    들어가는가"라는 다른 물음이고, 이쪽은 "들어가는 것들끼리 하나로 세는가"라는 물음이다.
  //    ⚠ 이 HOLD는 계산을 막지 않는다. 현재 구현은 종전 동작을 그대로 유지하고,
  //      확정되지 않았다는 사실만 화면 안내와 이 등록부에 남긴다.
  GEN2026_CRITICAL_DEDUCTIBLE_POOL_SCOPE: held(
    "GEN2026-CRITICAL-DEDUCTIBLE-POOL-SCOPE", "2026",
    [GEN5_CRITICAL_DEDUCTIBLE_CAP_TERMS, GEN5_THIRD_TABLE, GEN5_THIRD_LIMIT_BASIS],
    "확인됨(2026-09-05 제5조⑤ 직독, 인쇄 p.280): ①조문은 “상급종합병원·종합병원의 상해·질병 및 3대비급여 의료비(…제외) 중 공제금액”을 한 문장에서 열거하고 500만원이라는 수치는 한 번만 나온다. ②같은 조 제1항은 연간 보험가입금액을 “(1)상해비급여에 대하여… (2)질병비급여에 대하여…”로, 제3항은 3대비급여를 “각 비급여의료비별 보장한도”로 **나눌 때는 나눈다고 적는다.** 제5항에는 그런 분배 문언이 없다. ③따라서 열거된 의료비의 공제금액을 하나로 합산해 500만원을 적용하는 읽기가 문언에 가깝다. 막힌 이유: ①그 읽기를 확정해 주는 감독당국 유권해석·금융분쟁조정례·판례를 조회했으나 찾지 못했다(찾지 못한 것이지 존재하지 않는다고 단정하지 않는다). ②보험사 판매약관은 요약 성격의 보장한도 표 한 줄(“상급종합병원·종합병원 의료비 중 자기부담금은 최대 500만원(근골격계 이학요법치료·체외충격파치료, 주사료 제외)”)만 읽혔고 특별약관1 제5조 조문 전문에는 접근하지 못했다. 그 표는 표준약관이 “공제금액”이라 쓴 자리에 “자기부담금”을 쓴다. ③합산이라면 여러 보장종목이 같은 해에 걸릴 때 어느 순서로 소진되는지도 원문에 없다. 현재 구현: 일반 (1)(2) 입원은 priorAnnualDeductible로, 중증 MRI 입원은 priorAnnualInpatientDeductible로 **각각 500만원 한도를 따로** 계산한다(2026-09-05 실측). 이 HOLD는 그 동작을 바꾸지 않는다. 약관상 하나의 pool이라고도 서로 독립이라고도 확정하지 않는다. 계산기는 사용자가 입력한 누적 공제금액을 현재 경로에 적용할 뿐이며, 합산 범위 판단은 화면에서 가입 상품 약관과 보험사 안내로 넘긴다",
    "2026-09-05",
  ),

  // ⚠ 지급 보험금이 0원인 치료행위가 연 50회를 소진하는지는 직접 읽은 범위에서 판단 문언을
  //   찾지 못했다(<표1>·주)·④·⑦·제5조④·<보상기간 예시>).
  //   ⚠ ⑦·제5조④는 계속 중인 치료의 **이월 한도** 조항이다. 그 '보상한 횟수'를 연간 50회
  //     한도 일반의 소진 기준으로 확대해 인용하지 않는다.
  ...
  GEN2026_SPECIAL_ITEM_COUNT_ON_ZERO_PAY: held(
    ...
    "원문 사실: 금액 축의 누적 대상은 '지급한 금액'이고 1회의 단위는 치료·진단행위(주사료는 1회 통원·입원)다. <표1>은 '각 상해·질병 치료행위를 합산하여 … 50회까지 보상', 주)는 '각 치료횟수를 합산하여 최초 10회 보장'이라 하여 세는 대상을 **치료행위·치료횟수**로 적는다. 승인 구간의 카운터 단위는 '치료횟수'로 확정됐다(GEN2026-MSK-APPROVAL-COUNT-BASIS). ⚠ 제3조(3)⑦·제5조④는 계속 중인 치료의 **이월 한도** 조항이다 — 2026-09-04 재직독 결과 제5조④는 '…제7항에 따른 계속 중인 입원 또는 통원의 보장한도는…'으로 참조 조항이 명시돼 있다. 그 '보상한 횟수'를 연간 50회 한도 일반의 소진 기준으로 확대해 인용하지 않는다. 확인하지 못한 것: 공제금액이 의료비 이상이어서 지급 보험금이 0원인 치료행위가 <표1>의 '50회까지 보상'을 소진하는지. **직접 읽은 범위**(<표1>·주)·④·⑦·제5조④·<보상기간 예시>)에서 판단 문언을 찾지 못했다 — 유권해석·분쟁조정례·지급심사 기준은 조회하지 않았으므로 근거가 존재하지 않는다고 단정하지 않는다. 값을 만들지 않는다. ⚠ 이 HOLD는 3대비급여 횟수 축에만 해당한다 — 비중증 통원의 '통원 100일'은 별개 규칙(GEN2026-NONCRITICAL-OUTPATIENT-DAYS-ZEROPAY), 중증 통원의 '통원 100회'도 별개 규칙(GEN2026-CRITICAL-OUTPATIENT-VISITS-ZEROPAY)으로 각각 등록돼 있다. 셋을 하나로 합치지 않는다",
    "2026-09-03",
  ),
  ...
  //   특약2는 보상 단위가 '통원 1일당'이고 한도도 '통원 100일'이다.
  //   제5조④(p.309)의 '보상한 일수'는 **계속 중인 통원의 이월** 문언이라 단위 교차 확인에만 쓴다.
  ...
  GEN2026_NONCRITICAL_OUTPATIENT_ANNUAL_DAYS: confirmed5th(
    ...
    "매년 계약해당일부터 1년간 통원 100일을 한도로 한다(특약2 (1)①·(2)① 표, 인쇄 p.288·291). 단위는 '회'가 아니라 '일'이다. ⚠ 특약2 제5조④(p.309)의 '연간 보장한도(일수)·보상한 일수'는 **계속 중인 통원의 이월 계산** 문언이며, 이 연간 한도의 근거가 아니다 — 단위가 '일'이라는 것을 교차 확인하는 데만 쓴다. 이월 계산 자체는 구현하지 않았다",
  ),
  // ⚠ 공제금액이 의료비 이상이거나 연간 가입금액이 소진되어 **지급 보험금이 0원인 통원일**이
  //   이 100일을 소진하는지는 직접 읽은 범위에서 판단 문언을 찾지 못했다. 표는 '통원 100일'이라
  //   **통원 자체**를 센다. 값을 만들지 않는다.
  //   ⚠ 제5조④(p.309)의 '보상한 일수'를 반대 해석의 근거로 들지 않는다 — 계속 중인 입원·통원의
  //     **이월 한도** 전용 조항이다(아래 note 참조).
  GEN2026_NONCRITICAL_OUTPATIENT_DAYS_ON_ZERO_PAY: held(
    "GEN2026-NONCRITICAL-OUTPATIENT-DAYS-ZEROPAY", "2026",
    [GEN5_NONCRITICAL_OUTPATIENT_DAYS_TABLES, GEN5_NONCRITICAL_OUTPATIENT_DAYS_BASIS],
    "원문 사실: 연간 한도가 100일이고 보상 단위가 통원 1일당(외래 및 처방·조제비 합산)이다. 표(특약2 (1)①·(2)① p.288·291)가 세는 대상은 '통원' 자체다. ⚠ **특약2** 제5조④(p.309)는 계속 중인 입원·통원의 이월 한도 조항이다 — 2026-09-04 전문 재직독 결과 '제3조 (1)상해비급여 제3항 또는 제4항, (2)질병비급여 제2항 또는 제3항 및 (3)비급여 자기공명영상진단 제6항에 따른 계속 중인 입원 또는 통원의 보장한도는 연간 보장한도(보험가입금액)에서 … 연간 보장한도(일수)에서 직전 보험기간 종료일까지 보상한 일수를 차감한 잔여 일수를 한도로 적용합니다'로 참조 조항이 명시돼 있다. ⚠ 특약1 제5조④(p.280)는 참조 항이 '(3)3대비급여 제7항'이고 단위도 '횟수'라 **다른 문장**이다. 두 조항을 서로의 전문처럼 인용하지 않는다. '보상한 일수'는 그 이월 계산에만 쓰이며 연간 100일 한도의 소진 기준이 아니다. 확인하지 못한 것: 진료비는 있으나 지급 보험금이 0원인 통원일이 100일을 소진하는지. **직접 읽은 범위**에서 판단 문언을 찾지 못했다 — 유권해석·분쟁조정례·지급심사 기준은 조회하지 않았으므로 근거가 존재하지 않는다고 단정하지 않는다. 계산기의 현재 정책(보수적 차단): 두 해석을 독립 실행해 결과가 갈릴 때만 묶음 전체를 차단한다",
    "2026-09-03",
  ),
