...
console.log("\n[커밋 D·E] 계산·화면 무변경 (기준 30dee21)");
{
  const FROZEN: Record<string, string> = {
    ...
    //   한 글자도 바뀌지 않았다.
    "src/lib/insurance/engine/roomCharge2026.ts": "b6ccccb1d2f7b566600bee3db256f57e1ff6337efe99c8f04b9354f971494dfc",
    ...
    //   ⚠ G-34C에서 다시 갱신했다(종전 c08a59b7…). 세 가지다.
    //     (1) 다른 세대·다른 진입점의 축 25종을 **직접 호출**에서 거부한다 — 종전에는 접근자
    //         호출 0회로 조용히 버려졌다. 자리는 각 분기의 기존 preflight·stray **전부 뒤**라
    //         "치료유형을 고르세요"·"종별을 고르세요" 안내가 종전대로 먼저 나간다.
    //     (2) 종별을 쓰지 않는 두 경로(급여 입원 · 비급여 통원)에서 `tier`를 거부한다.
    //     (3) `coverage`·`visit`·`tier`를 호출당 **한 번만** 읽는다(종전 2~4회). 첫 사용
    //         시점에 읽는 메모이제이션이라 선행 차단 경로의 접근자 순서는 그대로다.
    //   ⚠ 산식·상수·상한 절삭·`ok()`/`pending()`의 반환값·기존 안내 문구는 그대로다.
    "src/lib/insurance/engine/generation2026.ts": "98a0d0612b5728b1ac81fae833153244295bbb3af7be595577377754a1c17ce1",
    ...
    //   ⚠ G-34C에서 다시 갱신했다(종전 33e405e2…). 세대 엔진들이 직접 호출에서 자기 소유가
    //     아닌 축을 거부하게 되면서, 라우터가 원본을 그대로 넘기면 그 새 가드가 이 파일의
    //     G-34A 가드를 **앞지른다**. 그래서 세대 소유 축만 **지연 투영**(getter 위임)해 넘기고
    //     원본은 stray 검사에만 쓴다. 2·3세대는 이 표가 `visit`까지 정확하므로 경로별 투영으로
    //     상대 경로의 축을 가려 라우터 안내가 그대로 먼저 나간다.
    //   ⚠ 소유권 표에서 `held: ["tier"]` 두 칸이 비었다 — G-34C가 확정 stray로 닫았고 판정은
    //     세대 엔진이 한다(라우터는 `coverage`까지 보는 조건을 쓸 수 없다).
    //   ⚠ 세대별 산식·상수·반환 계약·stray 안내 문구와 그 순서는 그대로다.
    "src/lib/insurance/engine/engine.ts": "aa858be2de0e8b99e30715d4cd726411871d37b0c1a77470e3da0a27ff0ac31b",
