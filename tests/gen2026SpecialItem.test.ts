...
console.log("\n[가드] 축·문구·출처의 금지형");
{
  ...
  // ⚠ 함수 **선언**을 기준으로 자른다. 이름이 주석이나 호출부에 먼저 나오면 첫 번째 split이
  //   엉뚱한 지점을 잡아 blocked()의 `totalInsurancePay: null`까지 범위에 들어온다(G-23에서 실제로 그랬다).
  check("전환 경로가 결과 금액을 손대지 않음",
    !/totalInsurancePay:/.test(eng.split("function calculateRoutedGeneral2026(")[1] ?? ""));
