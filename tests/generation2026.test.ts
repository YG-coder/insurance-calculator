...
// #4·5 중증 통원 Max(30%,3만) + 회당 20만 한도
{
  ...
  check("중증 통원 회당 20만 한도", big.insurancePay === 200000 && !!big.cappedBy?.includes("20만"), JSON.stringify(big));
}
...
// #7·9 비중증 입원 50% + 회당 300만 한도
{
  ...
  check("비중증 입원 50% + 회당 300만 한도", r.ownPay === 5000000 && r.insurancePay === 3000000 && !!r.cappedBy?.includes("300만"), JSON.stringify(r));
}
// #7·9 비중증 통원 Max(50%,5만) + 일당 20만 한도
{
  ...
  check("비중증 통원 일당 20만 한도", big.insurancePay === 200000 && !!big.cappedBy?.includes("20만"), JSON.stringify(big));
}
