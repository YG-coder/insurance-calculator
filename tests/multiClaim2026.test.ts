...

{
  ...
  const r = calculateMany2026({ cause: "disease", coverage: "non_benefit", severity: "critical", visit: "outpatient", amounts: [1_000_000, 1_000_000], outpatientCoverageLimit: 200_000 });
...
{
  const r = calculateMany2026({ cause: "disease", coverage: "non_benefit", severity: "non_critical", visit: "inpatient", amounts: [10_000_000, 10_000_000] });
...
{
  const r = calculateMany2026({ cause: "disease", coverage: "non_benefit", severity: "critical", visit: "inpatient", tier: "hospital", amounts: [10_000_000, 10_000_000], priorAnnualOwnPay: 4_000_000 });
...
}
{
  ...
  //   입력한 경우에만 적용된다.
  const r = calculateMany2026({ cause: "disease", coverage: "non_benefit", severity: "non_critical", visit: "outpatient", amounts: [1_000_000], priorAnnualInsurancePaid: 9_900_000, annualCoverageLimit: 10_000_000 });
...
// ── 2026-09-03 별표15 2026.5.6 직독 반영분 ──────────────────────────
{
  ...
  const r = calculateMany2026({
    cause: "disease", coverage: "non_benefit", severity: "critical", visit: "outpatient",
  ...
  const noLimit = calculateMany2026({
    cause: "disease", coverage: "non_benefit", severity: "critical", visit: "outpatient", amounts: [1_000_000],
  });
  ...
  const inpatient = calculateMany2026({
    cause: "disease", coverage: "non_benefit", severity: "non_critical", visit: "inpatient",
    amounts: [1_000_000], outpatientCoverageLimit: 100_000,
  });
  ...
  const nonCritical = calculateMany2026({
    cause: "disease", coverage: "non_benefit", severity: "non_critical", visit: "outpatient", amounts: [100_000],
  });
  ...
  const cap = calculateMany2026({
    cause: "disease", coverage: "non_benefit", severity: "critical", visit: "inpatient", tier: "hospital",
...
// ── 2026-09-03 실행 검증에서 잡힌 결함 회귀 방지 ────────────────────
{
  ...
  const nbOut = calculateMany2026({ cause: "disease", coverage: "non_benefit", severity: "critical", visit: "outpatient", amounts: [300_000] });
  check("비급여 통원에는 통원 안내가 붙음", nbOut.notes.join(" ").includes("통원 가입금액"));
  const nbIn = calculateMany2026({ cause: "disease", coverage: "non_benefit", severity: "critical", visit: "inpatient", amounts: [300_000] });
  ...
  // 99 / 100 / 101회 경계
  const at99 = calculateMany2026({ cause: "disease", coverage: "non_benefit", severity: "critical", visit: "outpatient", amounts: [300_000], priorAnnualOutpatientVisits: 99 });
  check("99회 사용 후 1건은 보상(100회째)", at99.lines[0].covered === true);
  const at100 = calculateMany2026({ cause: "disease", coverage: "non_benefit", severity: "critical", visit: "outpatient", amounts: [300_000], priorAnnualOutpatientVisits: 100 });
  check("100회 사용 후 1건은 제외(101회째)", at100.lines[0].covered === false);
  const across = calculateMany2026({ cause: "disease", coverage: "non_benefit", severity: "critical", visit: "outpatient", amounts: [300_000, 300_000], priorAnnualOutpatientVisits: 99 });
  check("경계를 걸치는 2건은 앞만 보상", across.lines[0].covered === true && across.lines[1].covered === false);

  // (5) 통원 가입금액 경계: 미입력 / 0 / 음수 / 10만 / 20만 초과
  const base = () => ({ cause: "disease" as const, coverage: "non_benefit" as const, severity: "critical" as const, visit: "outpatient" as const, amounts: [1_000_000] });
  ...
  const noAnnual = calculateMany2026({
    cause: "disease", coverage: "non_benefit", severity: "non_critical", visit: "outpatient",
  ...
  const annualOver = calculateMany2026({
    cause: "disease", coverage: "non_benefit", severity: "critical", visit: "inpatient",
...
// ── 상해/질병 보장축 분리 (특별약관1·2 제5조① — 각 축에 대해 따로 정해진다) ──────
{
  const injury = calculateMany2026({
    cause: "injury", coverage: "non_benefit", severity: "critical", visit: "inpatient",
  ...
  const injuryPrior = calculateMany2026({
    cause: "injury", coverage: "non_benefit", severity: "non_critical", visit: "outpatient",
    amounts: [1_000_000], priorAnnualInsurancePaid: 9_900_000, annualCoverageLimit: 10_000_000,
  });
